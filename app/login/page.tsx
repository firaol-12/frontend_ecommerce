...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  no_token: "Sign-in did not return a session. Please try again.",
  invalid_token: "Your session is invalid or expired. Please sign in again.",
  fetch_user_failed: "Could not load your profile. Please try again.",
};
...
function LoginPageInner() {
  ...
  // Show errors passed back from the backend via the URL,
  // e.g. /login?error=google_auth_failed&details=...
  useEffect(() => {
    const urlError = searchParams.get("error");
    const details = searchParams.get("details");
    if (!urlError) return;
    const base = ERROR_MESSAGES[urlError] || urlError.replace(/_/g, " ");
